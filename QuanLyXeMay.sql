...
begin 
	...
	begin 
		Raiserror(N'Thông tin đã trùng lặp, vui lòng nhập thông tin khác',16,1)		 
		return
	end 
	SET XACT_ABORT ON
	begin transaction 
...
as
begin
	if (@MaChiNhanh is null or @TenChiNhanh is null or @DiaChi is null)
	begin
		raiserror ('Vui lòng nhập thông tin đầy đủ',16,1)		
		return
	end
	if not exists (select * from ChiNhanh where MaChiNhanh = @MaChiNhanh)
	begin
		raiserror ('Chi nhánh này không tồn tại',16,1)		
		return
	end	
	if exists ( select * from ChiNhanh where ((MaChiNhanh != @MaChiNhanh) and (TenChiNhanh = @TenChiNhanh or DiaChi = @DiaChi)))
	begin 
		raiserror ('Tên chi nhánh hoặc địa chỉ đã trùng. Yêu cầu đặt tên chi nhánh và địa chỉ khác',16,1)		
		return
	end
	SET XACT_ABORT ON
	begin transaction 
...
begin 
	...
	if exists (select * from NhaCungCap where 
	TenNhaCungCap = @TenNhaCungCap or @DiaChi = DiaChi or DienThoai = @DienThoai or Email = @Email)
	begin 		
		Raiserror(N'Thông tin đã trùng lặp. Vui lòng kiểm tra lại thông tin',16,1)		
		return
	end 
	SET XACT_ABORT ON
	begin transaction
...
begin	
	...
	begin 
		raiserror (N'Không tìm thấy thông tin nhà cung cấp này',16,1)		
		return 
	end
	...
	if exists (select * from NhaCungCap where (MaNhaCungCap != @MaNhaCungCap) and 
	(TenNhaCungCap = @TenNhaCungCap or DiaChi = @DiaChi or DienThoai = @DienThoai or Email = @Email ))
	begin 		
		Raiserror(N'Thông tin đã trùng lặp .Vui lòng xem lại thông tin',16,1)		
		return
	end 
	SET XACT_ABORT ON
	begin transaction 
...
as
begin 
	if(@TenNhanVien is null or @DiaChi is null or @MaChiNhanh is null or @GioiTinh is null or @DienThoai is null or @NamSinh is null)
	begin 		
		Raiserror(N'Vui lòng nhập thông tin đầy đủ và chính xác',16,1)
		return 
	end 
	if exists(select * from NhanVien where DiaChi = @DiaChi or DienThoai = @DienThoai)
	begin 		
		Raiserror(N'Thông tin địa chỉ hoặc điện thoại đã trùng lặp',16,1)
		return 
	end 
...
as
begin	
	if(@TenNhanVien is null or @DiaChi is null or @MaChiNhanh is null or @GioiTinh is null or 
	@DienThoai is null or @NamSinh is null or @MaNhanVien is null)
	begin 		
		Raiserror(N'Vui lòng nhập thông tin đầy đủ và chính xác',16,1)
		return 
	end 
	...
	if exists(select * from NhanVien where (MaNhanVien != MaNhanVien) and 
	(DienThoai = @DienThoai or DiaChi = @DiaChi))
	begin 		
		Raiserror(N'Thông tin điện thoại và địa chỉ nhân viên này đã bị trùng lặp',16,1)
		return 
	end	
	SET XACT_ABORT ON
	begin transaction 
...
begin
	if not exists(select * from NhanVien where MaNhanVien = @MaNhanVien) 
	begin 
		raiserror (N'Thông tin nhân viên này không tồn tại',16,1)		
		return 
	end
	SET XACT_ABORT ON
	begin transaction 
...
begin 
	...
	if exists(select * from DongXe where MaLoaiXe = @MaLoaiXe and TenDongXe = @TenDongXe)
	begin
		raiserror(N'Thông tin đã trùng lặp, Vùi lòng xem lại thông tin',16,1)
		return
	end
	SET XACT_ABORT ON
	begin transaction
...
begin 
	...
	begin 
		raiserror (N'Không tìm thấy thông tin dòng xe này',16,1)
		return 
	end
...
begin 
	...
	if exists(select * from LoaiXe where TenLoaiXe = @TenLoaiXe) 
	begin
		raiserror(N'Tên loại xe đã trùng',16,1)
		return
	end
	SET XACT_ABORT ON
	begin transaction
...
begin 
	...
	begin 
		raiserror (N'Không tìm thấy thông tin loại xe này',16,1)
		return 
	end
	if(@TenLoaiXe in( select TenLoaiXe from LoaiXe where MaLoaiXe != @MaLoaiXe))
	begin 
		raiserror (N'Thông tin loại xe đã trùng lặp. Yêu cầu nhập tên loại xe khác',16,1)
		return 
	end
	SET XACT_ABORT ON
	begin transaction 
...
go
alter proc sp_ChinhSuaThongTinPhieuNhap @MaPhieuNhap int, @MaNhanVien int, @MaNhaCungCap int, @MaChiNhanh int, @NgayNhap date,@TongSoXeNhap int, @ThanhTienNhap float
as
begin
	if(@MaPhieuNhap is null or @MaNhanVien is null or @MaNhaCungCap is null or @MaChiNhanh is null or @NgayNhap is null or @TongSoXeNhap is null or @ThanhTienNhap is null)
	begin 
		raiserror (N'Vui lòng nhập đầy đủ thông tin',16,1)
		return
	end
	...
	begin 
		raiserror (N'Không tìm thấy thông tin phiếu nhập này',16,1)
		return
	end
	

	if exists (select * from PhieuNhap where (MaPhieuNhap != MaPhieuNhap))
	SET XACT_ABORT ON
	begin transaction
	begin try
		update PhieuNhap 
			set MaNhanVien = @MaNhanVien, MaNhaCungCap = @MaNhaCungCap, MaChiNhanh = @MaChiNhanh, NgayNhap = @NgayNhap, TongSoXeNhap = @TongSoXeNhap, ThanhTienNhap = @ThanhTienNhap
...
go
alter proc sp_ThemChiTietPhieuNhap @MaXe int,  @MaPhieuNhap int, @SoLuongNhap int, @DonGiaNhap float
as 
begin 
	...
	begin 
		rollback 
		Raiserror(N'Không tìm thấy thông tim phiếu nhập này',16,1)
	end
	if exists(select * from ChiTietPhieuNhap where MaPhieuNhap = @MaPhieuNhap and MaXe = @MaXe)
	begin 
		rollback 
		Raiserror(N'Thông tin đã trùng lặp. Vui lòng xem lại thông tin',16,1)
	end
...
go
alter proc sp_CapNhatThongTinChiTietPhieuNhap @MaXe int,  @MaPhieuNhap int, @SoLuongNhap int, @DonGiaNhap float
as
begin
	if(@MaXe is null or @MaPhieuNhap is null or @MaPhieuNhap is null or @SoLuongNhap is null or @DonGiaNhap is null)
	begin 		 
	...
	if not exists(select * from ChiTietPhieuNhap where MaXe = @MaXe and MaPhieuNhap = @MaPhieuNhap)
	begin 		 
		Raiserror(N'Không tìm thấy thông tin chi tiết mã phiếu nhập này. Vui lòng nhập thông tin chính xác',16,1)
		return
	end 
	if ( select count(*) from ChiTietPhieuNhap where MaXe = @MaXe and MaPhieuNhap = @MaPhieuNhap) >= 2
	begin 
		rollback 
		Raiserror(N'Vui lòng nhập kiểm tra lại thông tin xe và thông tin phiếu nhập đã bị trùng',16,1)
	end
...
begin
	...
	SET XACT_ABORT ON
	begin tran
	select * from khachhang where DienThoai like '%'+@dienthoai+'%' or TenKhachHang like '%'+@TenKH+'%'	
...
begin 
	...
	if exists (select * from KhachHang where DienThoai = @DienThoai AND TenKhachHang = @TenKhachHang)
	begin 
		raiserror('Thông tin khách hàng này đã trùng, yêu cầu kiểm tra lại thông tin',16,1)
		return 
	end
...
go
alter proc sp_CapNhatThongTinKhachHang @MaKhachHang int, @TenKhachHang nvarchar(100), @DienThoai nvarchar(100)
as
begin
	if(@MaKhachHang is null or @TenKhachHang is null or @DienThoai is null )
	begin 
		raiserror('Yêu cầu nhập thông tin đầy đủ',16,1)
		return 
	end
	...
	begin 
		raiserror('Không tìm thấy thông tin nhân viên này, vui lòng nhập thông tin chính xác',16,1)
		return 
	end
	if exists (select * from KhachHang where (MaKhachHang != MaKhachHang) and (DienThoai = @DienThoai))
	begin 
		raiserror('Thông tin khách hàng này đã trùng, yêu cầu kiểm tra lại thông tin',16,1)
		return 
	end
...
begin 
	...
	if exists(select * from PhieuBaoHanh where MaPhieuXuat = @MaPhieuXuat and SoKhung = @SoKhung and SoSuon = @SoSuon)
	begin
		Raiserror(N'Thông tin chiếc xe này đã được bảo hành',16,1)
		return
	end
	SET XACT_ABORT ON
	begin tran 
...
begin 
	...
	if exists(select * from PhieuBaoHanh where MaPhieuBaoHanh != @MaPhieuBaoHanh and
												MaPhieuXuat = @MaPhieuXuat and SoKhung = @SoKhung and SoSuon = @SoSuon)
	begin
		Raiserror(N'Thông tin chiếc xe này đã được bảo hành. Vui lòng nhập thông tin khác',16,1)
		return
	end
	SET XACT_ABORT ON
	begin tran 
...
as
begin 
	if(@MaPhieuXuat is null or @MaXe is null or @MaKhachHanh is null or @MaNhanVien is null or @MaChiNhanh is null or @MauXe is null or @MauXe is null or @NgayXuat is null or @ThanhTienXuat is null)
	begin 		
		Raiserror(N'Vui lòng nhập thông tin đầy đủ và chính xác',16,1)
		return
	end 
	...
	if exists (select * from PhieuXuat where MaPhieuXuat != @MaPhieuXuat and MaKhachHang = @MaKhachHanh and MaXe = @MaXe and MauXe = @MauXe and NgayXuat = @NgayXuat and MaChiNhanh = @MaChiNhanh)
		begin 
...
begin
	...
	if not exists (select * from Xe where MauXe = @MauXe and MaXe = @MaXe ) 
		begin
			raiserror ('Thông tin xe không tồn tại',16,1)
			rollback
		end
	SET XACT_ABORT ON
	begin tran
...
as
begin
	if(@madongxe is null or @tenxe is null or @MauXe is null or @machinhanh is null or @SoLuongHienCo is null or @TrongLuong is null
		or @thongtinbaohanh is null or @dongiaxe is null or @HinhAnh is null)
	begin
		raiserror (N'Vui lòng nhập đầy đủ thông tin',16,1)
		return
	end
...
go
create proc sp_CapNhatThongTinXe @Maxe int, @madongxe int, @tenxe nvarchar(100),@MauXe nvarchar(100), @machinhanh int, @SoLuongHienCo int, @TrongLuong int,
						@thongtinbaohanh nvarchar(100), @dongiaxe float,@namsanxuat int, @HinhAnh varbinary(max)
as
begin
	if(@madongxe is null or @tenxe is null or @MauXe is null or @machinhanh is null or @SoLuongHienCo is null or @TrongLuong is null
	...
	begin tran
	begin try		
		update xe set MauXe = @MauXe, MaDongXe = @madongxe, MaChiNhanh = @machinhanh, TenXe = @tenxe, GiaXe = @dongiaxe, TrongLuong = @TrongLuong,
		NamSanXuat = @namsanxuat,ThongTinBaoHanh = @thongtinbaohanh, SoLuongHienCo = @SoLuongHienCo, HinhAnh = @HinhAnh
		where MaXe = @Maxe		
		commit
	end try
	begin catch
...
go
create proc sp_TimKiemXe @value nvarchar(max), @ma int
as
begin
	...
	select * from Xe where TenXe like '%'+@value+'%' or MaXe = @ma 
end
...
begin 
	...
	open xecursor 
		...
		begin 
			if not exists (select * from @XeBanNhieuNhat where Maxe = @maxe)
			begin
			...
			begin	
				update @XeBanNhieuNhat set soLuongXe = soLuongXe +1 where Maxe = @maxe
			end 
			fetch next from xecursor into @maxe
		end 
	close xecursor
...
-- lấy 8 sản phẩm bán chạy nhất trong một tháng
go
create proc sp_8SanPhamBanChayTrongMotThang_HamGoi
as 
begin 
...
-- lấy thông tin xe tay côn
go
create proc sp_Lay4XeTayCon
as 
begin 
	select top(4)* from Xe inner join DongXe
						on DongXe.MaDongXe = xe.MaDongXe 
						join LoaiXe 
						on DongXe.MaLoaiXe = LoaiXe.MaLoaiXe
						where LoaiXe.MaLoaiXe = 1
end

exec sp_Lay8XeTayCon

-- lấy thông tin xe tay ga
go
create proc sp_Lay4XeTayGa
as 
begin 
	select top(4)* from Xe inner join DongXe
						on DongXe.MaDongXe = xe.MaDongXe 
						join LoaiXe 
						on DongXe.MaLoaiXe = LoaiXe.MaLoaiXe
						where LoaiXe.MaLoaiXe = 2
end


-- lấy thông tin xe số
go
create proc sp_Lay4XeSo
as 
begin 
	select top(4)* from Xe inner join DongXe
						on DongXe.MaDongXe = xe.MaDongXe 
...
----------------------------------XỬ LÝ ĐỒNG THỜI----------------------------------------
-----------------------------------------------------------------------------------------
-- ví dụ xử lý đồng thời của tìm kiếm xe

BEGIN TRAN
DECLARE @MaXe int
SELECT @MaXe = 1
FROM Xe with (updlock)
WHERE MaXe = @MaXe
ORDER BY MaXe
...
grant select,insert,update, delete on ChiNhanh to Admin
grant select,insert,update, delete on Xe to Nhanvien
grant select,insert,update, delete on DongXe to Nhanvien
grant select,insert,update, delete on LoaiXe to Nhanvien
grant select,insert,update, delete on PhieuDatXe to Nhanvien
grant select,insert,update, delete on PhieuBaoHanh to Nhanvien
grant select,insert,update, delete on PhieuNhap to Nhanvien
grant select,insert,update, delete on ChiTietPhieuNhap to Nhanvien
grant select,insert,update, delete on PhieuXuat to Nhanvien
grant select,insert,update, delete on ChiTietPhieuXuat to Nhanvien

Create Login Admin With Password = '123'
Create Login Nhanvien With Password = '123'

Create User admin1 For Login Admin
Create User nhanvien1 For Login NhanviengoSp_addRoleMember 'Admin', 'admin1'goSp_addRoleMember 'Nhanvien', 'nhanvien1'-----------------------------------------------------------------------------------------
------------------------------KẾT THỤC PHÂN QUYỀN NGƯỜI DÙNG-----------------------------
-----------------------------------------------------------------------------------------
...
select * from LoaiXe where MaLoaiXe = 1 
select * from Xe inner join DongXe
						on DongXe.MaDongXe = xe.MaDongXe 
						join LoaiXe 
						on DongXe.MaLoaiXe = LoaiXe.MaLoaiXe
						where LoaiXe.MaLoaiXe = 2
